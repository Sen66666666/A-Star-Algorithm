...
3. Algorithm Steps
 ...
 - Repeat the following steps until the Open list is empty:
   ...
   - For each neighboring node, if it is already in the closed list then it is ignored. If it is not in the open list then calculate its F, G and H values and add it to the open list. If it is already in the open list then update its G value to find a better path.
